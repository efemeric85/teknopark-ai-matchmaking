...
interface Event {
  id: string; name: string; date: string; duration: number; status: string;
}
interface User {
  id: string; full_name: string; company: string; title: string; email: string; event_id: string;
}
interface Match {
  id: string; event_id: string; user1_id: string; user2_id: string;
  round_number: number; status: string; started_at: string | null;
}
...
export default function AdminPage() {
  ...
  const loadParticipants = useCallback(async (eventId: string) => {
    const { data } = await supabase.from('users').select('*').eq('event_id', eventId).order('created_at', { ascending: true });
  ...
  const getUserById = (id: string): User | undefined => participants.find(p => p.id === id);
  ...
  const resetAllMatches = async () => {
    ...
    try {
      ...
      else { showMessage(data.message, 'success'); setMatches([]); }
    } catch (err: any) {
      showMessage('Bağlantı hatası: ' + err.message, 'error');
    } finally { setLoading(false); }
  };
  ...
  const getTimeLeft = (m: Match): number | null => {
    if (m.status !== 'active' || !m.started_at || !selectedEvent) return null;
    ...
    <div style={{ minHeight: '100vh', background: '#fafbfc', fontFamily: "'Inter', 'Segoe UI', sans-serif", padding: '20px' }}>
      <div style={{ maxWidth: '960px', margin: '0 auto' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          <p style={{ color: '#64748b', fontSize: '12px', textTransform: 'uppercase', letterSpacing: '2px', margin: '0 0 4px' }}>TEKNOPARK ANKARA</p>
          <h1 style={{ color: '#0f172a', fontSize: '22px', fontWeight: '700', margin: '0' }}>🛠️ Admin Paneli</h1>
        </div>

        {message && (
          <div style={{
            padding: '12px 16px', borderRadius: '10px', marginBottom: '16px', fontSize: '14px', fontWeight: '500',
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '10px' }}>
            <input type="text" placeholder="Etkinlik adı" value={eventName} onChange={e => setEventName(e.target.value)} style={inputStyle} />
            <input type="date" value={eventDate} onChange={e => setEventDate(e.target.value)} style={inputStyle} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <input type="number" placeholder="Süre (saniye)" value={eventDuration} onChange={e => setEventDuration(e.target.value)} style={inputStyle} />
            ...
            events.map(ev => (
              <div key={ev.id} onClick={() => setSelectedEvent(ev)} style={{
                ...
                <div>
                  ...
                  <span style={{ color: '#94a3b8', fontSize: '12px', marginLeft: '10px' }}>{ev.date} &bull; {ev.duration}s &bull; {ev.status}</span>
                </div>
                <button onClick={e => { e.stopPropagation(); deleteEvent(ev.id); }} style={btnDangerSmall}>🗑️ Sil</button>
              </div>
            ))
          }
        </div>
          ...
          <>
                ...
                participants.map(p => (
                  <div key={p.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #f1f5f9' }}>
                    <div>
                      ...
                      <span style={{ color: '#94a3b8', fontSize: '12px', marginLeft: '8px' }}>{p.company} &bull; {p.title}</span>
                    </div>
            ...
            <div style={cardStyle}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h3 style={{ ...sectionTitle, margin: '0' }}>🏆 Tur {currentRound || 0}</h3>
                {currentRound > 0 && (
                  <span style={{
              ...
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                {participants.length >= 2 && (
                  <button onClick={createAndStartMatches} disabled={loading} style={{ ...btnPrimary, opacity: loading ? 0.6 : 1 }}>
                ...
                {currentRound > 0 && (
                  <button onClick={resetAllMatches} disabled={loading} style={{
                    ...
                    fontSize: '14px', fontWeight: '600', cursor: 'pointer', background: '#fff', color: '#dc2626',
                  }}>🗑️ Tümünü Sıfırla</button>
                )}
              </div>
            </div>
            ...
            {currentRoundMatches.length > 0 && (
              <div style={cardStyle}>
                  ...
                  return (
                    <div key={m.id} style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isActive ? '12px' : '0' }}>
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                          <div style={{ background: '#fff', padding: '6px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                            <span style={{ fontWeight: '700', fontSize: '14px', color: '#0f172a' }}>{u1?.full_name || m.user1_id.slice(0, 8)}</span>
                            {u1 && <span style={{ color: '#94a3b8', fontSize: '11px', marginLeft: '6px' }}>{u1.company}</span>}
                          </div>
                          <span style={{ color: '#94a3b8', fontSize: '16px' }}>↔</span>
                          <div style={{ background: '#fff', padding: '6px 12px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                            <span style={{ fontWeight: '700', fontSize: '14px', color: '#0f172a' }}>{u2?.full_name || m.user2_id.slice(0, 8)}</span>
                            {u2 && <span style={{ color: '#94a3b8', fontSize: '11px', marginLeft: '6px' }}>{u2.company}</span>}
                          </div>
                        </div>
                        <span style={{
                          padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600', marginLeft: '12px', whiteSpace: 'nowrap' as const,
                      ...
                      {isActive && timeLeft !== null && !isExpired && (
                        <div style={{
                          background: '#fff', borderRadius: '10px', padding: '10px 16px', border: '1px solid #e2e8f0',
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <div style={{ width: '80px', height: '6px', background: '#f1f5f9', borderRadius: '3px', overflow: 'hidden' }}>
                              <div style={{
                                ...
                                background: getTimerColor(timeLeft), borderRadius: '3px', transition: 'width 1s linear',
                              }} />
                            </div>
                          </div>
                        </div>
                      ...
                      {isActive && isExpired && (
                        <div style={{ background: '#fff', borderRadius: '10px', padding: '10px 16px', border: '1px solid #fecaca', textAlign: 'center' }}>
                          <span style={{ color: '#dc2626', fontSize: '14px', fontWeight: '600' }}>⏰ Süre doldu!</span>
                        </div>
                      )}
                    </div>
                  );
                ...
                {unmatchedParticipants.length > 0 && (
                  <div style={{ marginTop: '8px', padding: '12px 16px', borderRadius: '10px', background: '#fffbeb', border: '1px solid #fde68a' }}>
                    <div style={{ fontSize: '13px', color: '#92400e', fontWeight: '600', marginBottom: '4px' }}>⏳ Beklemede ({unmatchedParticipants.length} kişi)</div>
                    {unmatchedParticipants.map(p => (
                      <div key={p.id} style={{ fontSize: '13px', color: '#a16207', padding: '2px 0' }}>{p.full_name} ({p.company})</div>
                    ))}
                    <div style={{ fontSize: '11px', color: '#a16207', marginTop: '6px' }}>Tek sayı katılımcı olduğu için bu turda eşleşmedi.</div>
                  </div>
                )}
              </div>
            )}
                ...
                {Array.from({ length: currentRound - 1 }, (_, i) => currentRound - 1 - i).map(roundNum => {
                  ...
                  return (
                    <div key={roundNum} style={{ marginBottom: '12px' }}>
                      <div style={{ fontSize: '13px', fontWeight: '600', color: '#64748b', marginBottom: '6px' }}>Tur {roundNum} ({roundMatches.length} eşleşme)</div>
                      {roundMatches.map(m => {
                        const u1 = getUserById(m.user1_id);
                        const u2 = getUserById(m.user2_id);
                        return (
                          <div key={m.id} style={{
                            display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                            padding: '8px 12px', borderRadius: '8px', marginBottom: '4px', background: '#f8fafc', border: '1px solid #e2e8f0', fontSize: '13px',
                          }}>
            ...
            <p style={{ textAlign: 'center', color: '#94a3b8', fontSize: '12px', marginTop: '8px' }}>Bu sayfa 5 saniyede bir otomatik güncelleniyor.</p>
          </>
        )}
      </div>
    </div>
...
const cardStyle: React.CSSProperties = { background: '#fff', borderRadius: '14px', padding: '20px', marginBottom: '16px', border: '1px solid #e2e8f0' };
const sectionTitle: React.CSSProperties = { fontSize: '15px', fontWeight: '600', color: '#334155', margin: '0 0 12px' };
...
const btnDangerSmall: React.CSSProperties = { background: '#fee2e2', border: 'none', borderRadius: '8px', padding: '6px 12px', color: '#dc2626', fontSize: '12px', fontWeight: '600', cursor: 'pointer' };
